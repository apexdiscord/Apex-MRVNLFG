...
export async function doAutomaticScaling(pluginData: PluginData<ScalingPluginType>) {
    ...
    let allScalingCategories = (await pluginData.state.scalingCategories.getAllScalingCategories()).sort((a, b) => a.id - b.id);
    ...
    channels.forEach((ch) => {
        if (ChannelType[ch.type] === "VoiceChannel") {
            ...
            if (allScalingCategories.find(x => x.category_id === ch.id) != null && manualUnhides.find(x => x.category_id === ch.id) == null) {
    ...
    if (pendingUnhide != null) {
        ...
        if ((everyonePerm.allow & pluginData.state.hidePermission) <= 0) {
            if ((everyonePerm.deny & pluginData.state.hidePermission) > 0) {
                await pendingUnhide.editPermission(pluginData.guild.id, everyonePerm.allow + pluginData.state.hidePermission, everyonePerm.deny - pluginData.state.hidePermission, "role");
            } else {
                await pendingUnhide.editPermission(pluginData.guild.id, everyonePerm.allow + pluginData.state.hidePermission, everyonePerm.deny, "role");
            }
        }
    ...
    pendingHides.forEach(async pend => {
        ...
        if ((everyonePerm.deny & pluginData.state.hidePermission) <= 0) {
          if ((everyonePerm.allow & pluginData.state.hidePermission) > 0) {
              await pend.editPermission(pluginData.guild.id, everyonePerm.allow - pluginData.state.hidePermission, everyonePerm.deny + pluginData.state.hidePermission, "role");
          } else {
              await pend.editPermission(pluginData.guild.id, everyonePerm.allow, everyonePerm.deny + pluginData.state.hidePermission, "role");
          }
        }
    });
}
