...
class Notification {
    ...
    constructor(modId: string, subjectId: string, channelId: string, endTime: number, persist: boolean, activeFollow: boolean) {
        this.modId = modId;
...
export class WherePlugin extends Plugin<IWherePluginConfig> {
    ...
    async notifyRequest(msg: Message, args: { user: User | UnknownUser, time?: number }): Promise<void> {
        ...
        msg.channel.createMessage(`If <@!${member.id}> joins or switches VC in the next ${humanizeDuration(timeout)} i will notify you`);
    ...
    async vcNotifyRequest(msg: Message, args: { channelId: string, time?: number }): Promise<void> {
        ...
        msg.channel.createMessage(`I will notify you of all changes in \`${channel.name}\` for the next ${humanizeDuration(timeout)}`);

        logger.info(`${msg.author.id}: ${msg.author.username}#${msg.author.discriminator} Requested notify for vc ${args.channelId}`);
    }
    ...
    async followRequest(msg: Message, args: { user: User | UnknownUser, time?: number, active?: boolean }): Promise<void> {
        const cfg: IWherePluginConfig = this.getConfig();
        const timeout: any = args.time != null ? args.time : cfg.where_timeout;
        ...
        if (!active) {
            msg.channel.createMessage(`I will let you know each time <@!${member.id}> switches channel in the next ${humanizeDuration(timeout)}`);
        } else {
            msg.channel.createMessage(`I will let you know each time <@!${member.id}> switches channel in the next ${humanizeDuration(timeout)}.\nI will also move you to the users channel, please join a voice channel now so that i can move you!`);
        }

        logger.info(`${msg.author.id}: ${msg.author.username}#${msg.author.discriminator} Requested follow for ${member.id} - Active Follow: ${active}`);
    }
    ...
    async followStopRequest(msg: Message, args: { user: User | UnknownUser }): Promise<void> {
        ...
        logger.info(`${msg.author.id}: ${msg.author.username}#${msg.author.discriminator} Requested notify/follow deletion for ${args.user.id}`);
    }
        ...
        this.activeNotifications.forEach(async notif => {
            if (notif.subjectId === member.id) {
                ...
                if (notif.endTime >= Date.now()) {
                    ...
                    sendWhere(this.guild, member, channel, "<@!" + notif.modId +
...
export async function sendWhere(guild: Guild, member: Member, channel: TextableChannel, prepend?: string): Promise<void> {
    ...
    if (voice == null) {
        ...
        channel.createMessage(`${prepend} <@!${member.id}> is in the following channel: \`${voice.name}\` ${getInviteLink(invite)}`);
    }
}